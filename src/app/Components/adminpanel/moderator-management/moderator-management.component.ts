...
export class ModeratorManagementComponent implements OnInit {
  ...
  constructor(private service: ModeratorService,
    ...
    private confimationService: DialogService) {
  }
  ...
  delete(id) {
    this.confimationService.openConfirmDialog(`Confirm deletion of ${this.dataSource[id].email}.`)
  ...
  deleteSelected() {
    this.confimationService.openConfirmDialog(`Confirm deletion of selected moderators.`)
    .afterClosed().subscribe(res => {
      if (res) {
        this.isLoadingResults = true;
